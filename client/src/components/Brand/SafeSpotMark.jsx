...
/** Product mark. Geometry lives in /safespot.svg (同构方标). */
  ...
  return (
    <span className={`inline-flex items-center min-w-0 ${stacked ? 'gap-2.5' : 'gap-2'} ${className}`}>
      ...
      <span className="leading-none min-w-0">
        <span
          className="block font-semibold tracking-tight text-fg"
          style={{ fontSize: Math.round(size * 0.58) }}
        >
          审微 · ShenWei
        </span>
        {stacked && (
          <span className="block mt-1 text-[10px] font-semibold tracking-[0.18em] text-accent">
            鹿溪联合创新实验室
          </span>
        )}
      </span>
